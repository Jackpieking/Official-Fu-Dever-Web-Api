    ...
    RemoveRolePermanentlyByRoleIdResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!isRoleFoundByRoleId)
        {
            return new()
            {
                StatusCode = RemoveRolePermanentlyByRoleIdResponseStatusCode.DEPARTMENT_IS_NOT_FOUND
            };
        }
        ...
        if (!isRoleTemporarilyRemoved)
        {
            return new()
            {
                StatusCode = RemoveRolePermanentlyByRoleIdResponseStatusCode.DEPARTMENT_IS_NOT_TEMPORARILY_REMOVED
            };
        }
                ...
                try
                {
                    ...
                    foreach (var foundUser in foundUsers)
                    {
                        await _unitOfWork.UserRepository.BulkUpdateByUserIdVer3Async(
                            ...
                            userRoleId: CommonConstant.App.DEFAULT_ENTITY_ID_AS_GUID,
                            cancellationToken: cancellationToken);
                    }*/
